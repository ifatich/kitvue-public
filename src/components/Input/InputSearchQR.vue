...
<template>
    ...
    <ModalComponent :persistent="false" :title="!isMobileSize() && `Scan QR`" :class="[`inputCameraQR`, isMobileSize() ? `mobile` : `desktop sticky`]" centered v-model="cameraOpen" hide-footer>
        <template #header v-if="isMobileSize()">
            <svg @click="cameraOpen= false" :id="`${$attrs.id}_closeCamera`"  width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path
       ...
       <template v-slot:body>
            ...
            <div v-if="isMobileSize()" class="cameraGuidance"></div>
            ...
            <svg v-if="isMobileSize()" @click="switchCamera()" class="switchCameraBtn" xmlns="http://www.w3.org/2000/svg" width="55" height="55" viewBox="0 0 55 55" fill="none">
