<template>
    <div>
        ...
        <table ref="dataTable" class="table rounded-corners" style="width: 100%">
            <thead>
                <tr>
                      <th v-for="column in filteredColumns" :key="column.data" @click="handleSort(column.data)">
            ...
            <tbody>
                <tr v-for="(item, index) in data" :key="index">
                    <td v-for="column in filteredColumns" :key="column.data" class="align-middle">
                         <template v-if="isDataArrayObject(item[column.data])">
                    ...
                    <div class="card-body">
                        ...
                        <div v-for="column in columns" :key="column.data" class="card-item">
                            ...
                            <span class="card-value" v-if="column.data !== leftContent && column.data !== rightContent">
                                ...
                                <template v-else-if="isDataImg(item[column.data])">
                                    <div class="text-link-data-table photo" @click="openModalSlider(index)">
                                        Lihat Foto
                                    </div>
                                </template>
                                <template v-else>
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            modalSliderOpen(newVal) {
              if(window.innerWidth < 560){
                if (newVal === true) {
                    this.$emit('update:modalTableOpen', false);
                } else {
                    this.$emit('update:modalTableOpen', true);
                }
              }
            },
        },
        methods: {
            getLimitedArrayDisplay(array) {
                if (array.length > 5 && !this.isDataExpanded) {
            ...
            openModalSlider(index) {
                this.$emit('update:selectedImgIndex', index);
                this.$emit('update:modalSliderOpen', true)
            },
            openModalTable(index){
                this.$emit('update:selectedDataIndex', index);
                this.$emit('update:modalTableOpen', true)
            },
        }, 
        computed: {
