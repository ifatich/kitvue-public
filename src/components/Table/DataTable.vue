                ...
                <tr v-for="(item, index) in data" :key="index">
                    <td v-for="column in columns" :key="column.data" class="align-middle">
                           <template v-if="isDataArrayhasArrayObject(item[column.data])">
                                <span v-html="getLimitedArrayObjectDisplay(item[column.data].rincianBarangJaminan, index)"></span>
                                <div v-if="item[column.data].rincianBarangJaminan.length > 5" class="d-flex gap-1 align-items-center text-link-data-table" @click="isDataObjectArrayExpanded[index] = !isDataObjectArrayExpanded[index]">
                        ...
                        <div v-for="column in columns" :key="column.data" class="card-item">
                            ...
                            <span class="card-value" v-if="column.data !== leftContent && column.data !== rightContent && !column.hideMobile">
                                ...
                                <template v-else-if="isDataArrayObject(item[column.data])">
                                    ...
                                    <div v-else class="text-link-data-table photo" @click="openModalHasilPemeriksaan(index)">
                                        Lihat Detail
                                    </div>
                                </template> 
                                <template v-else-if="isDataImg(item[column.data])">
                                    <div class="text-link-data-table photo" @click="openModalSlider(index, column.modalLayerIndex) ">
                                        Lihat Foto
                                    </div>
                                </template>
                                <template v-else>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            openModalSlider(index, layerIndex) {
                if (this.selectedSecondLayerIndex === undefined) {
                    ...
                    this.$emit('update:selectedSecondLayerIndex', index);
        ...
        computed: {
            ...
            // cek semua baris, kalau ada yang isDataArrayhasArrayObject maka kolom itu disembunyikan
            return !this.data.some(row => this.isDataArrayhasArrayObject(row[col.data]));
            });
        }
        },
